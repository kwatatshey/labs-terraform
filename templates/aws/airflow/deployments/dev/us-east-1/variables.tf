...
variable "routable_subnets" {
  type        = list(any)
  description = "List of subnet IDs that the redis and rds will be deployed in"
}

variable "eks_routable_subnets" {
  type        = list(any)
  description = "List of subnet IDs that the EKS cluster will be deployed in, normally same as routable subnets but can be used if recreation of the cluster would be triggered to override routable_subnets variable"
}
